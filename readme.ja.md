...
##### "max_age_in_days" `[int]`
- ルックアップを実行するときに考慮されるレポートの最大経過日数（１から３６５までの数値である必要があります）。​Default/デフォルルト = 365。

##### "minimum_confidence_score" `[int]`
- CIDRAMがＩＰアドレスをブロックするために必要な最小信頼スコア（０から１００までの数値である必要があります）。​Default/デフォルルト = 50。

##### "max_cs_for_captcha" `[int]`
- キャプチャを提供できるに許可された最大の信頼スコア（０から１００までの数値である必要があります）。​Default/デフォルルト = 10。

##### "minimum_total_reports" `[int]`
- CIDRAMがＩＰアドレスをブロックするために必要な合計レポートの最小数。​Default/デフォルルト = 1。

##### "report_back" `[bool]`
- CIDRAMがＡＰＩキーを使用して、検出された不正な動作をAbuseIPDBに報告できるようにしますか？​Default/デフォルルト = False。

##### "lookup_strategy" `[int]`
- どのリクエストに対してルックアップを実行する必要がありますか？

```
lookup_strategy
├─0 (しない。): モジュールをレポート目的でのみ使用する場合し、何も調べない場合は、これを使用しますください。
├─1 (すべてのリクエスト。): より厳格で徹底的ですが、制限と割り当てがはるかに迅速に満たされる可能性が高く、最も必要なときにサービスからロックアウトされる可能性があります。​また、ルックアップの結果は常にキャッシュされますが、それでも場合によってはウェブサイトのパフォーマンスに悪影響を与える可能性があります。​場合によっては必要になることもありますが、一般的にはお勧めしません。
└─2 (機密ページのみのリクエスト（例：ログイン・ページ、登録フォーム、など）。): それほど厳格で徹底的ではありませんが、割り当てと制限の点でより保守的であり、パフォーマンスに悪影響を与える可能性は低くなります。​ほとんどの場合、推奨される戦略です。
```

##### "build_profiles_from_usage_type" `[bool]`
- ＡＰＩによって返される使用タイプを使用してプロファイルを作成しますか？ True = はい（Default/デフォルルト）。 False = いいえ。

#### "bgpview" （カテゴリ）
BGPViewモジュールの設定（CIDRAMにASNおよび国コード検索機能を提供します）。

##### "blocked_asns" `[string]`
- BGPViewモジュールによってブロックされるASNのリスト。

##### "whitelisted_asns" `[string]`
- BGPViewモジュールによってホワイトリストに登録されるASNのリスト。

##### "blocked_ccs" `[string]`
- BGPViewモジュールによってブロックされる国（{{Links.ISO.3166}}二桁の国コードで識別）のリスト。

##### "whitelisted_ccs" `[string]`
- BGPViewモジュールによってホワイトリストに登録される国（{{Links.ISO.3166}}二桁の国コードで識別）のリスト。

#### "bunnycdn" （カテゴリ）
BunnyCDN互換性モジュールの設定。

##### "positive_action" `[string]`
- BunnyCDNからの要求に遭遇した場合、CIDRAMはどのアクションを実行する必要がありますか？ デフォルト・アクション = バイパスする。

```
positive_action
├─bypass ("バイパスをトリガーします。): 少なくとも１つのシグネチャがすでにトリガーされている限り、トリガーされたシグネチャの数から１カウントを減算します。​デフォルトのシグネチャ・ファイルによって引き起こされる可能性のある単純な偽陽性を処理するために推奨されます。"
├─greylist ("リクエストをグレイリストします。): トリガーされたシグネチャの数をリセットしますが、要求の処理を続行します。​リクエストをホワイトリストしたくないが、バイパスをトリガーするよりも実質的なものが必要な場合に推奨されます。"
└─whitelist ("リクエストをホワイトリストします。): トリガーされたシグネチャの数をリセットし、リクエストのそれ以上の処理を中止します。​CIDRAMがリクエストをブロックしないことを保証します。"
```
...
#### "projecthoneypot" （カテゴリ）
Project Honeypotモジュールの設定。
...
##### "max_age_in_days" `[int]`
- ルックアップを実行するときに考慮されるレポートの最大経過日数。​Default/デフォルルト = 365。

##### "minimum_threat_score" `[int]`
- CIDRAMがＩＰアドレスをブロックするために必要な最小脅威スコア（１から１００までの数値である必要があります）。​Default/デフォルルト = 10。

##### "max_ts_for_captcha" `[int]`
- キャプチャを提供できる最大の脅威スコア（１から１００までの数値である必要があります）。​Default/デフォルルト = 10。
...
#### "sfs" （カテゴリ）
Stop Forum Spam モジュールの設定。

##### "offer_captcha" `[bool]`
- このモジュールによってリクエストがブロックされた場合、CAPTCHAを提供できます。 Default（デフォルト設定） = True。 注：他のディレクティブをオーバーライドしません。 CAPTCHAを提供するには、すべてのディレクティブが同意し、必要なキーが構成されている必要があります。 CAPTCHAの提供が通常許可される場合、このディレクティブをfalseに設定すると、このモジュールによって特別にブロックされたリクエストに対してCAPTCHAが提供されないようにする方法が提供されます。
