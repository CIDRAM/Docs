...
次に、CIDRAMがそのアカウントでログインする際に２ＦＡコードを送信する場所を知るように、Ｅメール・アドレスをアカウントに関連付ける必要があります。​これを行うには、Ｅメール・アドレスをアカウントのユーザー名として使用する（例えば、`foo@bar.tld`）か、Ｅメール・アドレスを通常どおりＥメールを送信する場合と同じ方法でユーザー名の一部として含めます（例えば、`Foo Bar <foo@bar.tld>`）。
...
```
conflict_response
├─0 (リクエストをブロックしないでください。): リクエストが悪質であると確信できる場合にのみリクエストをブロックしたい場合、または誤検知に関して慎重を期したい場合（不要なトラフィックが時々通過してしまうという犠牲を払って）、これを選択してください。​リクエストが無害かどうか確信が持てない場合にリクエストをブロックしたい場合、または警戒を怠らない場合（時折誤検知が発生する可能性はあるもの）、利用可能な他のオプションのいずれかを選択してください。
