...
```
conflict_response
├─0 (リクエストをブロックしないでください。): リクエストが悪質であると確信できる場合にのみリクエストをブロックしたい場合、または誤検知に関して慎重を期したい場合（不要なトラフィックが時々通過してしまうという犠牲を払って）、これを選択してください。​リクエストが無害かどうか確信が持てない場合にリクエストをブロックしたい場合、または警戒を怠らない場合（時折誤検知が発生する可能性はあるものの）、利用可能な他のオプションのいずれかを選択してください。
