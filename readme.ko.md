...
3. 프런트엔드 백업 페이지에서, 내보내기를 선택하고, 구성 및 보조 규칙에 대한 상자를 체크한, 다음 확인을 눌러 현재 구성 및 보조 규칙의 백업을 다운로드합니다. 사용 가능한 구성과 보조 규칙 시스템에 몇 가지 변경 사항이 있었습니다. 예를 들어, "기록하지 마십시오" 동작은 보조 규칙에서 제거되었습니다 ("로깅 억제" 옵션을 대신 사용하여 동일한 결과를 얻을 수 있음). 이러한 변경 사항을 적용하려면 업그레이드 후 이 백업을 CIDRAM으로 다시 가져와야 합니다 (CIDRAM은 필요에 따라 보조 규칙과 구성을 자동으로 조정합니다).
...
7. 프런트엔드 백업 페이지에서, 가져오기를 선택하고, 구성 및 보조 규칙에 대한 상자를 체크한 다음, 파일을 선택하는 버튼을 클릭하고, 앞서 다운로드한 백업을 찾아 선택한 다음 확인을 눌러 해당 백업을 가져옵니다. CIDRAM은 이전 주요 버전의 백업에 있는 모든 보조 규칙과 구성을 자동으로 조정하여 가져오는 주요 버전에 맞게 조정합니다.
