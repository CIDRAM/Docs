...
##### "timezone" `[string]`
- 사용할 시간대를 지정하는 데 사용됩니다 (예: Africa/Cairo, America/New_York, Asia/Tokyo, Australia/Perth, Europe/Berlin, Pacific/Guam 등). PHP가 이를 자동으로 처리하도록 하려면 "SYSTEM"을 지정하세요.
...
유용한 팁: 시간 형식 placeholder를 사용하여 로그 파일 이름에 날짜/시간 정보를 붙일 수 있습니다. 사용할 수 있는 시간 형식 placeholder는 <a onclick="javascript:toggleconfigNav('generalRow','generalShowLink')" href="#config_general_time_format"><code dir="ltr" translate="no">general➡time_format`</a>에 표시되어 있습니다.
...
##### "log_rotation_limit" `[int]`
- Log rotation은 한 번에 존재하는 로그 파일 개수를 제한합니다. 새 로그 파일이 만들어질 때 총 로그 파일 개수가 지정된 제한을 초과하면 지정된 동작이 수행됩니다. 여기에서 원하는 제한을 지정할 수 있습니다. 값을 0으로 설정하면 log rotation이 비활성화됩니다.

##### "log_rotation_action" `[string]`
- Log rotation은 한 번에 존재하는 로그 파일 개수를 제한합니다. 새 로그 파일이 만들어질 때 총 로그 파일 개수가 지정된 제한을 초과하면 지정된 동작이 수행됩니다. 여기에서 원하는 동작을 지정할 수 있습니다.
...
##### "usemode" `[int]`
- CAPTCHA는 언제 제공해야 합니까? 여기에서 지원되는 각 공급자에 대한 기본 동작을 지정할 수 있습니다.
...
참고: 허용 목록에 있거나 확인되고 차단되지 않은 요청은 보안 문자를 작성할 필요가 없습니다.

또한 참고: CAPTCHA는 봇 및 다양한 종류의 악성 자동 요청에 대한 유용한 추가 보호 계층을 제공할 수 있지만 악의적인 사람에 대한 보호는 제공하지 않습니다.

요청은 보조 규칙을 통해 "사용 예정으로 표시"할 수 있습니다.

요청이 "민감한" 요청으로 간주되는지 여부는 <a onclick="javascript:toggleconfigNav('generalRow','generalShowLink')" href="#config_general_sensitive"><code dir="ltr" translate="no">general➡sensitive`</a>에 의해 결정됩니다.

"서명 제한"은 <a onclick="javascript:toggleconfigNav('captchaRow','captchaShowLink')" href="#config_captcha_signature_limit"><code dir="ltr" translate="no">captcha➡signature_limit`</a>에 의해 결정됩니다.
...
##### 6.3.2 사용자 지정 서명 파일 저장 및 활성화

CIDRAM v2 및 이전 버전의 경우, 저장소에는 두 개의 파일이 있습니다: `ipv4_custom.dat.RenameMe` 및 `ipv6_custom.dat.RenameMe`. 사용자 지정 서명을 해당 파일에 저장하거나, 원하는 이름으로 새 파일을 만들어 사용자 지정 서명을 저장할 수도 있습니다. CIDRAM v2 및 이전 버전의 경우, 사용자 지정 서명 파일은 vault 디렉터리의 최상위 폴더에 저장해야 합니다.

CIDRAM v3 이상 버전에서는, 사용자 지정 서명용 파일이 미리 준비되어 있지 않지만, 원하는 이름으로 새 파일을 만들어 사용자 지정 서명을 저장할 수 있습니다. CIDRAM v3 이상 버전에서는, 사용자 지정 서명 파일은 CIDRAM의 "signatures" 디렉터리에 저장해야 합니다.

사용자 지정 서명 파일을 "활성화"하려면, 구성 파일의 "ipv4" 또는 "ipv6" 구성 지시문에서 해당 파일을 인용해야 합니다 (사용자 지정 서명 파일이 IPv4 서명용인지 IPv6 서명용인지에 따라 다름).

CIDRAM v2 및 이전 버전의 경우, "ipv4" 및 "ipv6"은 "signatures" 구성 범주에서 찾을 수 있습니다.

CIDRAM v3 이상 버전에서는, "ipv4" 및 "ipv6"이 "components" 구성 범주에서 찾을 수 있습니다.
...
- [7.2보다 오래된 PHP 버전에서 CIDRAM v2~v4을 사용하고 싶습니다; 도울 수 있니?](#user-content-MINIMUM_PHP_VERSION_V3)
...
예. 가장 쉬운 방법은 BGPView 모듈 또는 IP-API 모듈을 설치하고 필요에 따라 구성하는 것입니다.
...
#### <a name="MINIMUM_PHP_VERSION_V3"></a>7.2보다 오래된 PHP 버전에서 CIDRAM v2~v4을 사용하고 싶습니다; 도울 수 있니?

아니오. PHP≥7.2은 CIDRAM v2~v4의 최소 요구 사항입니다.
...
CIDRAM은 CAPTCHA를 지원합니다. 올바르게 작동하려면 API 키가 필요합니다. 기본적으로 비활성화되어 있지만 필요한 API 키를 구성하여 활성화 할 수 있습니다. 활성화되면 서비스와 CIDRAM 또는 사용자 브라우저 간에 통신이 발생할 수 있습니다. 여기에는 사용자의 IP 주소, 사용자 에이전트, 운영 체제 및 요청에 사용할 수 있는 기타 세부 정보와 같은 정보 통신이 포함될 수 있습니다.
...
CIDRAM은 [쿠키](https://ko.wikipedia.org/wiki/HTTP_%EC%BF%A0%ED%82%A4)를 코드베이스의 두 지점에 설정합니다. 1. `lockto` 설정 방식에 따라 CIDRAM은 사용자가 CAPTCHA를 완료한 시점을 기억하기 위해 쿠키를 설정할 수 있습니다. 2. 사용자가 프론트 엔드에 성공적으로 로그인하면, CIDRAM은 후속 요청에 대해 사용자를 기억할 수 있도록 쿠키를 설정합니다 (즉, 쿠키는 로그인 세션에 대해 사용자를 인증하는 데 사용됩니다).
