...
##### “error_log_stages”
- 执行链中应该记录错误的阶段列表。
- *标准： "Tests,Modules,SearchEngineVerification,SocialMediaVerification,OtherVerification,Aux,Reporting,Tracking,RL,CAPTCHA,Statistics,Webhooks,Output"*
...
- *可用选项： `GitHub,macmathan.info,BitBucket,NotABug,GoogleDNS`*
...
*cookie警告通常与reCAPTCHA一起显示。​为了禁用它，添加了此配置指令（为了帮助，例如，隐藏任何使用CIDRAM的指示）。​但是，我强烈建议大多数用户（特别是欧盟的用户）保持启用。*
...
*也可以看看：​[“PDO DSN”是什么？如何能PDO与CIDRAM一起使用？](#HOW_TO_USE_PDO)*
...
CIDRAM提供了一个可选模块，以使用[BGPView](https://bgpview.io/)执行ASN和国家代码查找。​这些查询提供了根据其ASN或来源国家阻止或将在白名单上放请求的功能。​默认情况下该模块不是安装，但如果选择安装该模块，则可以根据模块的预期用途将用户的IP地址与BGPView API共享。
