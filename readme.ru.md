...
CIDRAM можно бесплатно загрузить здесь:
- [GitHub](https://github.com/CIDRAM/CIDRAM).
- [Bitbucket](https://bitbucket.org/Maikuolan/cidram).
- [Codeberg](https://codeberg.org/Maikuolan/CIDRAM).

Этот документ и его различные переводы можно найти здесь:
...
Вы можете настроить конфигурацию, и настроить какие CIDR-ы заблокированы, путем изменения файла конфигурации и/или ваши сигнатур файлы.
...
├─UALC ("Пользовательский агент (в нижнем регистре)")
...
```
http_response_header_code───[Стандарт]─[Юридический]─[Запрещенный]
...
__1.__ При включении «тихого режима» будет использоваться HTTP-сообщение о состоянии, определяемое `general➡silent_mode_response_header_code` (оно имеет наивысший приоритет).

__2.__ При запрашивающий объект был заблокирован из-за превышения лимита нарушений, будет использовано HTTP-сообщение о статусе «запрещенный».

__3.__ При блокировке из-за ограничения скорости будет использоваться код 429, а при блокировке из-за конфликтов ресурсов будет использоваться сообщение о состоянии HTTP, определенное `signatures➡conflict_response` (ограничение скорости и конфликты ресурсов имеют равный приоритет в этом контексте).

__4.__ При блокировке из-за вспомогательного правила, которое устанавливает «переопределение кода статуса HTTP», будет использоваться это переопределение кода статуса HTTP.

__5.__ При блокировке по юридическим причинам (т.е., при блокировке из-за пользовательской сигнатуры, в которой используется сокращенное слово «юридический») будет использоваться HTTP-сообщение о состоянии «юридический».

__6.__ Для всех остальных заблокированных запросов будет использоваться HTTP-сообщение о состоянии «стандарт» (оно имеет самый низкий приоритет).
...
##### «request_proxy» `[string]`
- Если хотите чтобы исходящие запросы отправлялись через прокси-сервер, укажите этот прокси-сервер здесь. Если нет, оставьте это поле пустым.
...
##### «email_notification_address» `[string]`
- Если решили получать уведомления от CIDRAM по электронной почте, например, когда срабатывают определенные вспомогательные правила, здесь вы можете указать адрес получателя этих уведомлений.

##### «email_notification_name» `[string]`
- Если решили получать уведомления от CIDRAM по электронной почте, например, когда срабатывают определенные вспомогательные правила, здесь вы можете указать имя получателя этих уведомлений.
...
##### «custom_header» `[string]`
- Вставляется как HTML в начале всех фронтенд страниц. Это может быть полезно, если хотите включить логотип веб-сайта, персонализированный заголовок, скрипты, или подобное на всех таких страницах.

##### «custom_footer» `[string]`
- Вставляется как HTML в конце всех фронтенд страниц. Это может быть полезно, если хотите включить юридическое уведомление, контактную ссылку, деловую информацию, или подобное на всех таких страницах.
...
##### «usemode» `[int]`
- Когда следует предлагать CAPTCHA? Здесь вы можете указать предпочтительное поведение для каждого поддерживаемого поставщика.
...
Примечание: Запросы из белого списка или проверенные и неблокированные запросы никогда не требуют выполнения CAPTCHA.

Также обратите внимание: CAPTCHA может обеспечить полезный дополнительный уровень защиты от ботов и различных вредоносных автоматических запросов, но не обеспечивает никакой защиты от вредоносных люди.
...
##### «hcaptcha_sitekey» `[string]`
- Если хотите использовать hCaptcha с CIDRAM, вам потребуется ввести здесь значение. Если нет, можете проигнорировать его.
...
##### «friendly_sitekey» `[string]`
- Если хотите использовать Friendly Captcha с CIDRAM, вам потребуется ввести здесь значение. Если нет, можете проигнорировать его.
...
##### «turnstile_sitekey» `[string]`
- Если хотите использовать Cloudflare Turnstile с CIDRAM, вам потребуется ввести здесь значение. Если нет, можете проигнорировать его.
...
##### «custom_header» `[string]`
- Вставляется как HTML в начало всех страниц «доступ запрещен». Это может быть полезно, если хотите включить логотип веб-сайта, персонализированный заголовок, скрипты, или подобное на всех таких страницах.

##### «custom_footer» `[string]`
- Вставляется как HTML в конце всех страниц «доступ запрещен». Это может быть полезно, если хотите включить юридическое уведомление, контактную ссылку, деловую информацию, или подобное на всех таких страницах.
...
Если хотите разделить ваши собственные сигнатуры на отдельные секции, Вы можете идентифицировать эти отдельные секции к скрипт путем добавления «теги секций» сразу же после того как сигнатуры каждого секция, наряду с названием вашей сигнатур секция (смотрите пример ниже).
...
Применение марка YAML в файлах сигнатур совершенно не обязательно (то есть, можете использовать его, если хотите сделать это, но не обязаны делать это), и способен использовать большинство (но не все) директивы конфигурации.
...
К тому же, если хотите CIDRAM полностью игнорировать некоторые конкретные секций в любой из сигнатур файлы, Вы можете использовать файл `ignore.dat` чтобы указать какие секций игнорировать. На новой строка, писать `Ignore`, а затем пробел, а затем название секций что Вы хотите CIDRAM игнорировать (смотрите пример ниже).

```
Ignore Секция 1
```

Это также может быть достигнуто с помощью фронтенда, предоставленного на странице «списка секций» на фронтенд в CIDRAM.

##### 6.3.1 ВСПОМОГАТЕЛЬНЫЕ ПРАВИЛА

Если Вы считаете, что писать собственные файлы сигнатуры или настраиваемые модули слишком сложны для вас, более простой альтернативой может быть использование фронтенда, предоставляемого страницей «вспомогательных правил» на фронтенде CIDRAM. Выбрав соответствующие параметры и указав детали о конкретных типах запросов, вы можете указать CIDRAM, как реагировать на эти запросы. «Вспомогательные правила» выполняются после того, как все файлы сигнатур и модули уже завершены.

##### 6.3.2 СОХРАНЕНИЕ И АКТИВАЦИЯ ПОЛЬЗОВАТЕЛЬСКИХ ФАЙЛОВ СИГНАТУРЫ

Для CIDRAM v2 и более ранних в хранилище вы найдете два файла: `ipv4_custom.dat.RenameMe` и `ipv6_custom.dat.RenameMe`. В этих файлах можно сохранять пользовательские сигнатуры, или, если хотите, можно создать новые файлы для ваших пользовательских сигнатурей, называя их как угодно. Для CIDRAM v2 и более ранних файлы пользовательских сигнатурей необходимо сохранять в корневом каталоге vault.

В CIDRAM v3 и более поздних отсутствуют предустановленные файлы для пользовательских сигнатурей, но вы также можете создавать новые файлы для своих пользовательских сигнатурей, называя их как угодно. Для CIDRAM v3 и выше файлы пользовательских сигнатурей необходимо сохранять в подготовленной директории «signatures» вашей установки CIDRAM.

Для «активации» пользовательских файлов сигнатурей необходимо указать их в директивах конфигурации «ipv4» или «ipv6» вашего конфигурационного файла (в зависимости от того, предназначены ли пользовательские файлы сигнатурей для IPv4 или IPv6).

В CIDRAM v2 и более ранних директивы конфигурации «ipv4» и «ipv6» находятся в категории конфигурации «signatures».

В CIDRAM v3 и выше директивы конфигурации «ipv4» и «ipv6» находятся в категории конфигурации «components».
...
- [Я хочу использовать CIDRAM v2~v4 с версией PHP ниже 7.2; Вы можете помочь?](#user-content-MINIMUM_PHP_VERSION_V3)
...
Да. Самый простой способ сделать это - Установить модуль BGPView или модуль IP-API и настроить его в соответствии с вашими потребностями.
...
#### <a name="MINIMUM_PHP_VERSION_V3"></a>Я хочу использовать CIDRAM v2~v4 с версией PHP ниже 7.2; Вы можете помочь?

Нет. PHP≥7.2 является минимальным требованием для CIDRAM v2~v4.
...
Если хотите, чтобы `file3.php` выполнялся первым, вы могли бы добавить что-то вроде `aaa:` перед именем файла:
...
Такая же ситуация, когда файл деактивирован. Наоборот, если хотите, чтобы файл выполнялся последним, вы могли бы добавить что-то вроде `zzz:` перед именем файла. В любом случае вам не нужно будет переименовывать указанный файл.
...
│               ├Может соединиться с хостом и номером порта.
│               │
│               └Вам следует обратиться к документации Oracle, если хотите
│                использовать это.
├─odbc
│ │
│ │      ╔═════╗
...
│         ├Может соединиться с хостом и номером порта.
│         │
│         └Вам следует обратиться к документации ODBC/DB2, если хотите
│          использовать это.
├─pgsql
│ │
│ │            ╔═══════╗      ╔══╗        ╔═════╗
...
Если не уверены, что использовать для какой-либо конкретной части вашего DSN, попробуйте сначала выяснить, работает ли она как есть, ничего не меняя.
...
Пользовательский агент: Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/66.0.3359.181 Safari/537.36
...
Если хотите сделать еще один шаг, не позволяя полностью регистрировать определенные типы информации, это также можно сделать. На странице конфигурации обратитесь к директиве конфигурации `fields`, чтобы контролировать, какие поля будут отображаться в записях журнала и на странице «Доступ Закрыт».
...
CIDRAM устанавливает файлы [куки](https://ru.wikipedia.org/wiki/Cookie) в двух точках своей кодовой базы. Во-первых, в зависимости от настроек `lockto`, CIDRAM может установить cookie-файл чтобы запомнить когда пользователь прошел проверку CAPTCHA. Во-вторых, когда пользователь успешно входит в фронтенд, CIDRAM устанавливает куки, чтобы иметь возможность запоминать пользователя для последующих запросов (то есть, куки используются для аутентификации пользователя в сеансе фронтенд).
...
Чтобы просмотреть список изменений внесенных в v3 (например, добавленные функции, удаленные функции, и т.д), обратитесь к [журналу изменений v3](https://github.com/CIDRAM/CIDRAM/blob/v3/Changelog.md#v300).

#### 10.1 Обновление до CIDRAM v4 с версии более ранней чем CIDRAM v3

См. выше: Рекомендуется новая установка.

#### 10.2 Обновление до CIDRAM v4 с CIDRAM v3

1. Сначала перейдите на страница обновлений фронтенда, и если есть какие-либо доступные обновления, обязательно установите их все. Это гарантирует что любой недавно написанный код, который может потребоваться для корректного обновления основных версий, будет доступен для обновления, а также помогает сократить объем работы которую придется выполнить обновителю при последующем обновлении основных версий.

2. Перейдите на страница конфигурации фронтенда и найдите __`frontend➡remotes`__. В списке, где вы видите `/v3/`, измените его на `/v4/`. Нажмите «Обновить» чтобы сохранить конфигурацию. Это изменение указывает программе обновления на необходимость ориентироваться на предполагаемую основную версию при поиске обновлений.

3. Перейдите на страницу резервного копирования фронтенда. Выберите экспорт, отметьте флажками конфигурацию, статистику, и вспомогательные правила, а затем нажмите «ОК» чтобы загрузить резервную копию. Произошли некоторые изменения. Например, действие «не регистрировать» было удалено из вспомогательных правил (вместо него можно использовать опцию «подавить ведение журнала» для достижения того же результата). После обновления вам потребуется импортировать эту резервную копию обратно в CIDRAM.

4. Вернитесь на страницу обновлений фронтенда. Теперь должны появиться обновления для новой основной версии. Чтобы избежать тайм-аутов, перед обновлением всех компонентов сначала попробуйте обновить только ​​ядро или фронтенд CIDRAM (поскольку оба они взаимозависимы, обновление одного из них в любом случае должно автоматически обновить оба). Поскольку структура страницы и стили CSS для фронтенда существенно изменились между основными версиями, после обновления он может сначала показаться неисправным, но это не так. Перейдите на любую другую страницу фронтенда и нажмите Ctrl+F5 чтобы попытаться выполнить жесткое обновление (т.е., обновление, при котором браузер загружает новую копию стилей CSS и других периферийных элементов, а не использует свой кэш). После этого структура страницы и стили CSS должны отображаться правильно. Если этого не произошло, попробуйте очистить кэш браузера.

5. После этого вы можете обновить все остальное. Вернитесь на страницу обновлений фронтенда и, если вы видите кнопку в верхней части страницы, нажмите «Обновить все».

6. Чтобы убедиться в отсутствии каких-либо устаревших или текущих неудачных обновлений, а также поврежденных файлов в установке и убедиться, что все работает как надо, нажмите «Починить все». Реальная проблема может возникнуть крайне редко, но лучше перестраховаться.

7. Вернитесь на страницу резервного копирования фронтенда. Выберите импорт, отметьте флажками конфигурацию, статистику, и вспомогательные правила, нажмите кнопку чтобы выбрать файл, найдите и выберите ранее загруженную резервную копию, и нажмите «ОК» чтобы импортировать эту резервную копию. CIDRAM автоматически выполнит все необходимые корректировки для учета изменений.

8. На этом вы завершили обновление. Поскольку вы успешно обновили основные версии, вам возможно захочется вкратце изучить страницу конфигурации фронтенда, поскольку изменения внесенные новой основной версией (например, для ознакомления с новыми функциями). Новая основная версия не вносит никаких изменений в файлы сигнатур, модули, или события, поэтому вам не нужно беспокоиться об этом при обновлении.

Чтобы просмотреть список изменений внесенных в v4 (например, добавленные функции, удаленные функции, и т.д), обратитесь к [журналу изменений v4](https://github.com/CIDRAM/CIDRAM/blob/v4/Changelog.md#v400).

---


Последнее обновление: 18 Апреля 2026 г (2026.04.18).
