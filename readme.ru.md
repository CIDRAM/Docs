...
#### «abuseipdb» (Категория)
Конфигурация для модуля AbuseIPDB.

##### «api_key» `[string]`
- Пожалуйста, введите ваш ключ API здесь.
...
##### «max_age_in_days» `[int]`
- Максимальный возраст в днях, за который отчеты будут учитываться при выполнении поиска (должно быть числом от 1 до 365). Стандарт = 365.

##### «minimum_confidence_score» `[int]`
- Минимальная оценка достоверности, необходимая для того, чтобы CIDRAM заблокировала IP-адрес (должно быть числом от 0 до 100). Стандарт = 50.

##### «max_cs_for_captcha» `[int]`
- Максимальная оценка достоверности, разрешенный для обслуживания CAPTCHA (должно быть числом от 0 до 100). Стандарт = 10.

##### «minimum_total_reports» `[int]`
- Минимальное общее количество отчетов, необходимых для того, чтобы CIDRAM заблокировала IP-адрес. Стандарт = 1.

##### «report_back» `[bool]`
- Разрешить CIDRAM сообщать об обнаруженном плохом поведении обратно в AbuseIPDB, используя ваш ключ API? Стандарт = False.

##### «lookup_strategy» `[int]`
- Для каких запросов следует выполнять поиск?

```
lookup_strategy
├─0 (Ничего.): Если вы хотите использовать модуль
│ только для отчетов, ничего не ища,
│ используйте это.
├─1 (Каждый запрос.): Более строгий и тщательный, но также с
│ большей вероятностью приведет к более
│ быстрому выполнению ограничений и квот,
│ что может привести к блокировке службы,
│ когда она больше всего нужна. Также,
│ несмотря что результаты поиска всегда
│ кэшируются, в некоторых случаях это все
│ же может негативно сказаться на
│ производительности веб-сайта. В
│ некоторых случаях может быть необходимо,
│ но обычно не рекомендуется.
└─2 (Запросы только для конфиденциальных страниц (например, войти страницы, регистрационные формы, и т.д.).): Менее строгий и тщательный, но более
  консервативный с точки зрения квот и
  ограничений, и с меньшей вероятностью
...
##### «build_profiles_from_usage_type» `[bool]`
- Создавать профили, используя тип использования, возвращаемый API? False = Нет. True = Да. Стандарт = True.

#### «bgpview» (Категория)
Конфигурация для модуля BGPView (предоставляет средство поиска ASN и кода страны для CIDRAM).

##### «blocked_asns» `[string]`
- Список ASN, которые будут заблокированы при соответствует модулем BGPView.

##### «whitelisted_asns» `[string]`
- Список ASN для включения в белый список при соответствует модулем BGPView.

##### «blocked_ccs» `[string]`
- Список стран (идентифицируются их двухзначными кодами стран {{Links.ISO.3166}}), которые будут заблокированы при соответствует модулем BGPView.

##### «whitelisted_ccs» `[string]`
- Список стран (идентифицируются их двухзначными кодами стран {{Links.ISO.3166}}) для включения в белый список при соответствует модулем BGPView.

#### «bunnycdn» (Категория)
Конфигурация для модуля совместимости BunnyCDN.

##### «positive_action» `[string]`
- Какое действие должен выполнить CIDRAM при получении запроса от BunnyCDN? Стандартное действие = Обходите.

```
positive_action
├─bypass ("Обходить.): Если задействована хотя бы одна сигнатур,
│ из числа задействованных сигнатурей
│ вычитается один счет. Рекомендуется для
│ работы с простыми ложными
│ срабатываниями, которые могут быть
│ вызваны файлами сигнатур стандардию."
├─greylist ("Серый список.): Сбрасывает число задействованных
│ сигнатурей, но продолжает обработку
│ запроса. Рекомендуется, если вы не хотите
│ вносить запрос в белый список, но вам
│ нужно что-то более существенное, чем
│ обход."
└─whitelist ("Белый список.): Сбрасывает число задействованных
  сигнатурей, и прерывает дальнейшую
  обработку запроса. Гарантирует, что CIDRAM
  никогда не заблокирует запрос."
```
...
#### «projecthoneypot» (Категория)
Конфигурация для модуля Project Honeypot.
...
##### «max_age_in_days» `[int]`
- Максимальный возраст в днях, для которого отчеты будут учитываться при выполнении поиска. Стандарт = 365.

##### «minimum_threat_score» `[int]`
- Минимальная оценка угрозы, необходимая для того, чтобы CIDRAM заблокировала IP-адрес (должно быть числом от 1 до 100). Стандарт = 10.

##### «max_ts_for_captcha» `[int]`
- Максимальный оценка угрозы, разрешенный для обслуживания CAPTCHA (должно быть числом от 1 до 100). Стандарт = 10.
...
#### «sfs» (Категория)
Конфигурация модуля Stop Forum Spam.

##### «offer_captcha» `[bool]`
- Когда запросы блокируются этим модулем, может использоваться CAPTCHA. Стандарт = True. Примечание: Другие директивы не переопределяются. Для выдачи CAPTCHA должны быть согласованы все директивы, предоставил необходимые ключи, и т.д. В случае, если CAPTCHA обычно разрешается выдавать, установка этой директивы в false обеспечивает способ предотвращения выдачи CAPTCHA для запросов, заблокированных конкретно этим модулем.
...
- [Я хочу использовать CIDRAM v3 с версией PHP ниже 7.2; Вы можете помочь?](#MINIMUM_PHP_VERSION_V3)
...
Последнее обновление: 30 Июня 2022 г (2022.06.30).
