...
- *Доступные варианты: `GitHub,macmathan.info,BitBucket,NotABug,GoogleDNS`*
...
*Примечание: reCAPTCHA защищает только от машинных вызовов, а не от людей.*
...
*Эта директива конфигурации добавляется по запросу для пользователей, которые хотят отключить предупреждение о файлах куки, которое обычно отображается рядом с reCAPTCHA (чтобы помочь например, скрыть любые признаки того, что CIDRAM используется). Тем не менее, я настоятельно рекомендую, чтобы большинство пользователей (особенно те, которые базируются в ЕС) оставляли его включенным.*
...
Последнее обновление: 29 Апреля 2021 г (2021.04.29).
