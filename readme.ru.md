...
├─CAPTCHAs-Failed ("Попытки CAPTCHA – Не удалось (%s)!")
├─CAPTCHAs-Passed ("Попытки CAPTCHA – Успех (%s)!")
...
##### 6.2.1 КАК «СПЕЦИАЛЬНО ОТМЕТИТЬ» СИГНАТУР СЕКЦИЙ ДЛЯ ИСПОЛЬЗОВАНИЯ С hCaptcha

Когда «usemode» является 2 или 5, чтобы «специально отметить» сигнатур секций для использования с hCaptcha, запись включается в сегменте YAML для этого секция сигнатуры (смотрите пример ниже).
...
Последнее обновление: 21 Августа 2025 г (2025.08.21).
