...
3. 转到备份页面。​选择导出，勾选配置和辅助规则的复选框，按“OK”即可下载当前配置和辅助规则的备份。​可用配置和辅助规则系统发生了一些变化。​例如，“不记录”操作已从辅助规则中删除（可以使用“禁止日志记录”选项来实现相同的效果）。​为了适应这些变化，升级后，将此备份导回CIDRAM，以便根据需要自动调整辅助规则和配置。
